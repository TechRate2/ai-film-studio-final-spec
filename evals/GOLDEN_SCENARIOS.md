...
Handoff counterexamples (TASK-001/002, R-079): spec nested without root instruction bridges; inaccessible/stale pin; a newer code tree with old passing logs; prose acceptance criterion omitted from receipt; deleted failing test; a partial checkpoint mistaken for COMPLETE; spec CI green while app tests fail. All must prevent false acceptance. Positive case: fresh session resolves one canonical revision, resumes the same incomplete task, preserves failed evidence and reports exact next actions without another paid submission. Test root discovery in actual coding-agent setup and bootstrap/CI checks in the implementation repository; canonical link checks alone are insufficient.
