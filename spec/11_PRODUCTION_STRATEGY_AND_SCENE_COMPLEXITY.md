...
## Comparable route decisions
Compare feasible routes for the same deliverable, hard constraints and cost scope. Include required reference/keyframe work, video, dialogue/voice, assembly/postprocessing and expected repair burden where scoped measurements support it; retain category breakdown in CostEstimate. Keep generation-only accepted-second economics separate from end-to-end project cost. Existing media is not assumed free of editing, licensing or compute cost. Known estimates are nonnegative; null/UNKNOWN is not zero or a winning rank.

Select quality tier, duration and reference count for their measured benefit to the required result, not their advertised maximum. Distinguish native generation from upscaling/enhancement; a higher export size cannot fix incorrect identity, acting or story. Do not add unrelated references to obtain a pricing discount. Before spend, refresh price/entitlement against the exact compiled request and retain estimate assumptions/profile versions; a quote marked estimated is not guaranteed final cost. Spec/44's bounded-liability reservation still applies, including automatic-duration requests. If no defensible bound exists, block that route instead of pretending a hard cap is enforceable.
