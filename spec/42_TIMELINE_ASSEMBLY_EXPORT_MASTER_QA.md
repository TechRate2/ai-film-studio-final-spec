...
## Localization retiming and subtitle appearance
For original-video localization, `spec/47_SOURCE_VIDEO_LOCALIZATION.md` owns source-to-target mapping, subtitle-only outputs and no-generation constraints. Timeline compiler consumes that explicit map, never estimates speech timing from nominal frame rate. Optional style fields outline_width_px, outline_color_rgba, background_color_rgba, bold, italic and line_spacing_px default respectively to 0, transparent, transparent, false, false and 0 for legacy styles. Colors are parsed RGBA values at semantic validation, not arbitrary renderer code. Style IDs pin real licensed font assets; missing glyph/shaping support blocks the affected exposed output or requires a user-accepted fallback. Preview and export share compiler/style versions. SRT exports timing/text without rich styling; ASS/burn-in preserve supported styling with explicit capability checks.
