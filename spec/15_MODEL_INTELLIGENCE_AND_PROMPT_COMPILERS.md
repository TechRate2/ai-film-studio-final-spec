...
Claim-local promotion is mandatory: SUPPORTED or UNSUPPORTED requires VERIFIED_SOURCE or PRODUCT_OBSERVED evidence, a source locator/version, verification timestamp and explicit scope. REPO_REPORTED, SYNTHESIS and UNVERIFIED claims remain UNKNOWN even when their reported value is populated. PRODUCT_OBSERVED requires positive sample count, nonempty sample refs and confidence; every sample ref resolves to an inspectable ledger record. A source may be model-native or provider-wide: state that scope explicitly, and keep model_version null only for genuinely model-independent provider claims. A model-specific claim must pin its exact model version before resolution. Account/region scope belongs in the referenced evidence record and must match the requested route.

MEASURED at profile level requires at least one qualifying observed claim, including behavior/bias or provider exposure/billing/limits/timeout claims where applicable; it never promotes other unknown claims. UNVERIFIED or FAMILY model profiles cannot be routable, and routable model profiles require an exact version. Documentation may establish a documented capability without a paid sample; it does not establish measured acceptance probability or duration reliability. Source/sample authenticity, scope match and freshness remain ingestion/resolver checks, not facts established by nonempty JSON strings.
